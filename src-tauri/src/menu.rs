...
pub fn create_main_menu(app: &AppHandle) -> tauri::Result<()> {
    ...
    let theme_submenu = SubmenuBuilder::new(app, "Theme")
        .item(&MenuItem::with_id(app, "theme_light", "Light (Default)", true, None::<&str>)?)
        .item(&MenuItem::with_id(app, "theme_dracula", "Dracula", true, None::<&str>)?)
        .item(&MenuItem::with_id(app, "theme_nord", "Nord", true, None::<&str>)?)
        .build()?;
    ...
    app.on_menu_event(move |app_handle, event| {
        match event.id().as_ref() {
            ...
            theme_id if theme_id.starts_with("theme_") => {
                ...
                if let Some(theming) = app_handle.try_state::<Arc<dyn crate::traits::ThemingEngine>>() {
                    theming.set_active_theme(&theme_name);
                    
                    if let Some(orchestrator) = app_handle.try_state::<Arc<dyn crate::traits::TabOrchestrator>>() {
                        let tab_ids = orchestrator.get_tab_ids();
                        for tab_id in tab_ids {
                            let _ = orchestrator.inject_theme_into_tab(app_handle, &tab_id, &theme_name);
                        }
                    }
                    
                    // Persist to config
                    if let Some(config_state) = app_handle.try_state::<crate::config::LotionConfig>() {
